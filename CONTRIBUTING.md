...
Explore Traces uses GitHub to manage reviews of pull requests:
...
import (
	...
	"github.com/grafana/Explore Traces/modules/overrides"
	"github.com/grafana/Explore Traces/pkg/validation"
)
...
- **Metrics**: Explore Traces is instrumented with [Prometheus metrics](https://prometheus.io/). It emits RED metrics for most
  services and backends. .
- **Logs**: Explore Traces uses the [go-kit level logging library](https://pkg.go.dev/github.com/go-kit/kit/log/level) and emits
  logs in the `key=value` (logfmt) format.
- **Traces**: Explore Traces uses the [Jaeger Golang SDK](https://github.com/jaegertracing/jaeger-client-go) for tracing instrumentation.
  As of this writing, only the read path of Explore Traces is instrumented for tracing.

### Testing

We try to ensure that most functionality of Explore Traces is well tested.
...
- Finally, we write integration tests that often test the functionality of the ingest and query path of Explore Traces as a
...
Using a debugger can be useful to find errors in Explore Traces code. This [example](./example/docker-compose/debug)
shows how to debug Explore Traces inside docker-compose.
...
Anyone can help with Explore Traces's documentation by writing new content, updating existing content, or creating an issue.
...
Explore Traces documentation is located in the `docs` directory. The `docs` directory has three folders:
...
Explore Traces uses a CI action to sync documentation to the Grafana web site. The CI is
